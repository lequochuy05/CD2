...
Tài liệu này trình bày toàn bộ cấu trúc hệ thống, luồng dữ liệu (Dataflow) thời gian thực và sự phối hợp giữa quy trình dựng bản đồ **SLAM (Cartographer)** và hệ thống điều hướng **Nav2 (A* & DWA)** trên robot tự hành **Turtlebot3 Burger** mô phỏng trong Gazebo.
...
graph TD
    ...
    SLAM[Cartographer / SLAM Toolbox Node]:::algorithm
    GLOBAL[Global Planner: Thuật toán A*]:::algorithm
    LOCAL[Local Planner: Thuật toán DWA]:::algorithm
    
    MOTORS[OpenCR 1.0 / Gazebo: Động cơ DYNAMIXEL]:::actuation

    %% Dataflow arrows
    LIDAR -- "/scan (LiDAR 2D Scan)" --> SLAM
    SENSORS -- "Raw Odometry & IMU" --> EKFa
...
3.  **So khớp quét (Scan Matching):** Thuật toán **Cartographer** so khớp các tia Laser quét được ở thời điểm $t$ và $t-1$ để tính toán sự dịch chuyển hình học của robot trong không gian.
4.  **Tối ưu hóa vòng lặp (Loop Closure):** Khi robot quay trở lại một vị trí đã từng đi qua trong mê cung, Cartographer tự động nhận diện vùng không gian quen thuộc, thực hiện tối ưu đồ thị (Pose Graph Optimization) để kéo thẳng toàn bộ bản đồ và loại bỏ sai số trôi tích lũy.
5.  **Xuất bản đồ Lưới chiếm dụng (Occupancy Grid Map):** Bản đồ cuối cùng được xuất bản dưới dạng mảng 2D lên topic `/map`. Mỗi ô lưới đại diện cho một không gian kích thước thực tế (ví dụ: $5cm \times 5cm$) với ba trạng thái:
    *   **$0$ (Free Space):** Vùng trống trống robot có thể đi qua.
...
*   **Inflation Layer (Lớp phình):** Tự động tạo ra một vùng chi phí giảm dần bao quanh các bức tường. Càng sát tường chi phí di chuyển càng cao. Điều này đảm bảo thuật toán tìm đường tránh xa tường, tránh việc robot bị cọ sát vào tường do sai số vật lý.
...
1.  **A\* Planner (Global Path Planning):**
    *   Dựng đồ thị từ Costmap.
    *   Thực hiện thuật toán **A\*** tìm kiếm chuỗi ô có tổng chi phí $F(n) = G(n) + H(n)$ nhỏ nhất từ vị trí hiện tại đến lối thoát mê cung.
    *   Xuất bản đường đi tối ưu dưới dạng danh sách tọa độ hình học lên topic `/plan`.
2.  **DWA Controller (Local Path Control):**
    *   Nhận đường đi mẫu `/plan` và dữ liệu quét LiDAR tức thời `/scan`.
    *   Thực hiện thuật toán **DWA** liên tục ở tần số 20Hz để sinh mẫu các cặp vận tốc ($v, \omega$).
    *   Chọn quỹ đạo cua tối ưu nhất bám theo đường đi `/plan` mà không đâm vào các vật cản đột xuất.
    *   Gửi lệnh `/cmd_vel` điều khiển tốc độ góc và tốc độ thẳng trực tiếp đến các bánh xe của Turtlebot3.
...
| :--- | :--- | :--- | :--- | :--- |
| `/scan` | `sensor_msgs/msg/LaserScan` | LiDAR (Gazebo) | Cartographer, Nav2 | Dữ liệu khoảng cách quét chướng ngại vật của mê cung. |
| `/odom` | `nav_msgs/msg/Odometry` | EKF Node | Cartographer | Vị trí ước lượng tương đối dựa trên động cơ và IMU. |
| `/map` | `nav_msgs/msg/OccupancyGrid` | Cartographer | Nav2 Planner, RViz | Bản đồ lưới mê cung 2D (0: trống, 100: tường, -1: tối). |
| `/initialpose` | `geometry_msgs/msg/PoseWithCovarianceStamped` | `maze_solver.py` / RViz | AMCL / Cartographer | Thiết lập tọa độ khởi xuất phát mặc định cho robot. |
...
| `/cmd_vel` | `geometry_msgs/msg/Twist` | Nav2 Local Planner (DWA) | OpenCR / Gazebo | Lệnh tốc độ di chuyển bánh xe ($v, \omega$) gửi tới robot. |
